...
typedef UserNameChangedCallback = void Function(int id, String newName);

class UserNameChangedEventNotifier {
  int _count = 0;
  List<UserNameChangedCallback?> _listeners =
      List<UserNameChangedCallback?>.filled(0, null);
  ...
  void addListener(UserNameChangedCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<UserNameChangedCallback?>.filled(1, null);
      } else {
        final List<UserNameChangedCallback?> newListeners =
            List<UserNameChangedCallback?>.filled(_listeners.length * 2, null);
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<UserNameChangedCallback?> newListeners =
          List<UserNameChangedCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(UserNameChangedCallback listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final UserNameChangedCallback? _listener = _listeners[i];
  ...
  void notifyListeners(int id, String newName) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () sync* {
            yield DiagnosticsProperty<UserNameChangedEventNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<UserNameChangedCallback?> newListeners =
            List<UserNameChangedCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final UserNameChangedCallback? listener = _listeners[i];
          if (listener != null) {
