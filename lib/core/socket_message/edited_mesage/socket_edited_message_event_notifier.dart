...
import 'socket_edited_message_model.dart';

typedef SocketEditedMessageCallback = void Function(
    SocketEditedMessageModel message);

class SocketEditedMessageEventNotifier {
  int _count = 0;
  List<SocketEditedMessageCallback?> _listeners =
      List<SocketEditedMessageCallback?>.filled(0, null);
  ...
  void addListener(SocketEditedMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<SocketEditedMessageCallback?>.filled(1, null);
      } else {
        final List<SocketEditedMessageCallback?> newListeners =
            List<SocketEditedMessageCallback?>.filled(
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<SocketEditedMessageCallback?> newListeners =
          List<SocketEditedMessageCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(SocketEditedMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final SocketEditedMessageCallback? _listener = _listeners[i];
  ...
  void notifyListeners(SocketEditedMessageModel message) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () sync* {
            yield DiagnosticsProperty<SocketEditedMessageEventNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<SocketEditedMessageCallback?> newListeners =
            List<SocketEditedMessageCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final SocketEditedMessageCallback? listener = _listeners[i];
          if (listener != null) {
