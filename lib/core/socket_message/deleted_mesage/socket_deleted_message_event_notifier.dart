...
import 'socket_deleted_message_model.dart';

typedef SocketDeletedMessageCallback = void Function(
    SocketDeletedMessageModel message);

class SocketDeletedMessageEventNotifier {
  int _count = 0;
  List<SocketDeletedMessageCallback?> _listeners =
      List<SocketDeletedMessageCallback?>.filled(0, null);
  ...
  void addListener(SocketDeletedMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<SocketDeletedMessageCallback?>.filled(1, null);
      } else {
        final List<SocketDeletedMessageCallback?> newListeners =
            List<SocketDeletedMessageCallback?>.filled(
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<SocketDeletedMessageCallback?> newListeners =
          List<SocketDeletedMessageCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(SocketDeletedMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final SocketDeletedMessageCallback? _listener = _listeners[i];
  ...
  void notifyListeners(SocketDeletedMessageModel message) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () sync* {
            yield DiagnosticsProperty<SocketDeletedMessageEventNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<SocketDeletedMessageCallback?> newListeners =
            List<SocketDeletedMessageCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final SocketDeletedMessageCallback? listener = _listeners[i];
          if (listener != null) {
