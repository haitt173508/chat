...
import 'socket_typing_model.dart';

typedef SocketTypingCallback = void Function(SocketTypingModel message);

class SocketTypingEventNotifier {
  int _count = 0;
  List<SocketTypingCallback?> _listeners =
      List<SocketTypingCallback?>.filled(0, null);
  ...
  void addListener(SocketTypingCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<SocketTypingCallback?>.filled(1, null);
      } else {
        final List<SocketTypingCallback?> newListeners =
            List<SocketTypingCallback?>.filled(_listeners.length * 2, null);
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<SocketTypingCallback?> newListeners =
          List<SocketTypingCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(SocketTypingCallback listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final SocketTypingCallback? _listener = _listeners[i];
  ...
  void notifyListeners(SocketTypingModel message) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () sync* {
            yield DiagnosticsProperty<SocketTypingEventNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<SocketTypingCallback?> newListeners =
            List<SocketTypingCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final SocketTypingCallback? listener = _listeners[i];
          if (listener != null) {
